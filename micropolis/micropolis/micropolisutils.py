...
class MicropolisView:
    ...
    def applyToClippedTiles(self, x, y, width, height, onTilesBegin, onRowBegin, onTile, onRowEnd, onTilesEnd):

        left = max(x, 0)
        top = max(y, 0)
        right = min(x + width, micropolisengine.WORLD_X)
        bottom = min(y + height, micropolisengine.WORLD_Y)
    ...
    def renderClippedTilesAsHtml(
        ...
        def onTile(x, y, tile, flags):
            out.append(
                """<td width="16" class="t" style="background-position: %dpx %dpx"/>""" % (
                    -16 * (tile % 16),
                    -16 * (tile / 16),
                ))
